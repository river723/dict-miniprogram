...
| 9 | **新建并部署 `tts` 云函数**（新函数，IDE 右键 `cloudfunctions/tts` → 上传并部署；超时配 10s） | 未部署时点发音会提示"发音播放失败"；部署后首次点击有短暂延迟、第二次起秒播 |
...
- ✅ **发音音频域名（2026-09-20 已实现，待部署云函数后真机验收）**：
  原实现直接播 `https://dict.youdao.com/dictvoice`，属**第三方域名** —— 配置合法域名需要往
  对方服务器根目录放校验文件，我们做不到，所以**开发版能响、体验版/正式版必然没声音**。
  现已改为 **`tts` 云函数代拉 → base64 返回 → 前端写本地文件 → 播**（不落云存储）：
  新增 `cloudfunctions/tts/`（超时 10s，带实例级 300 条缓存 + 512KB 体积上限 + 格式嗅探）
  与 `miniprogram/utils/tts.js`（本地文件缓存 + LRU 400 条 + 并发去重 + 缓存失效重试 +
  `prefetch` 预热），`study.js` / `word-detail.js` / `dictionary-word-detail.js` 三处已改接。
  **⚠️ 必须重新部署 `tts` 云函数才生效**（前端改动重新编译即可）。
  **配额核算（实测，见 `.workbuddy/tts_storage_estimate.cjs`）**：词库均匀抽 60 词，
  音频均值 **13.29 KB**（中位 10.54 / P90 12.98）→ base64 后约 17.7 KB，远低于云函数返回体
  1MB 上限；4801 词全缓存 62MB，但本地文件上限 200MB，故 **LRU 只留 400 个（约 5MB）**。
  ⚠️ **云函数调用 20 万/月仍是瓶颈**（约 67 个重度用户打满）→ 本地缓存是这套方案的**前提**，
  不是优化；用户量上来后再在 `tts` 云函数内加"云存储命中返回 fileID"分支，前端与页面代码都不用动。
  **验收清单**：① 首次点发音有 0.5~1.5s 延迟、之后秒播 ② 关闭网络后已听过的词仍能播
  ③ 自动朗读（翻词即念）不弹 toast ④ 真机（非开发者工具）确认出声
