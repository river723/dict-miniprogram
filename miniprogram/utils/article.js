...
export function markWords(plain, words = []) {
  ...
  if (list.length === 0) return [{ text: plain, hit: false }];
  const re = new RegExp(`\\b(${list.join('|')})\\b`, 'gi');
  const out = [];
  let last = 0;
  let m = re.exec(plain);
  while (m) {
    if (m.index > last) out.push({ text: plain.slice(last, m.index), hit: false });
    out.push({ text: m[0], hit: true });
    last = m.index + m[0].length;
    m = re.exec(plain);
  }
  if (last < plain.length) out.push({ text: plain.slice(last), hit: false });
  return out;
}
